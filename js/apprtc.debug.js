function callSummary(){
	var avgquality = (_qrate/_Count).toFixed(2);
	var avgJitter = (_jitter/_Count).toFixed(2);
	alert('Your average audio quality rating over the call: '+ avgquality +'\nand Jitter: '+avgJitter);
}
function stats(){
	_pc.getStats().then(function(report){
	  report.forEach(function(obj){
	    if(obj.type == "ssrc" && obj.mediaType == "audio"){
		if(obj.bytesReceived){ //received
		 $('#rx_jitter').innerHTML = obj.googJitterReceived;
		 $('#rx_packetloss').innerHTML = obj.packetsLost;
		 _jitter = _jitter + Number(obj.googJitterReceived);
                 if(_aPBR === 0){
                  _aPBR = parseInt(obj.bytesReceived);
                 }else{
                  var rx_bw = Number( (((parseInt(obj.bytesReceived) - _aPBR)/5) / 1024).toFixed(1) );
                  $('#rx_bw').innerHTML = rx_bw;
  		  _aPBR = parseInt(obj.bytesReceived); 
		 }
		 _aQ = (rx_bw?rx_bw:0); //save current downstream rate
		 console.log('rx: ','googJitterReceived: '+obj.googJitterReceived,' packetloss: '+obj.packetsLost,' googCurrentDelayMs: '+obj.googCurrentDelayMs);
		}
		if(obj.bytesSent){ // Sent
		  $('#tx_jitter').innerHTML = obj.googJitterReceived;
		  $('#tx_packetloss').innerHTML = obj.packetsLost;
                  $('#rtt').innerHTML = obj.googRtt;
                 if(_aPBS === 0){
                  _aPBS = parseInt(obj.bytesSent);
                 }else{
                  $('#tx_bw').innerHTML =  Number( (((parseInt(obj.bytesSent) - _aPBS)/5) / 1024).toFixed(1) ) ;
                  _aPBS = parseInt(obj.bytesSent);
                 }
		  console.log('tx: ','googJitterReceived: '+obj.googJitterReceived,' packetloss: '+obj.packetsLost,' googRtt: '+obj.googRtt);
		}
           // Update quality status
              var qrate = Number(_aQ.toFixed(2));
              _Count++;
	      _qrate = _qrate + qrate;
              $('#qrate').innerHTML = (qrate > 5)?5:qrate;
              if(qrate <= 2){ // Since each downstream packet value ranges from 0-6 kB/s
               _aQDC++;
              }
	      if(_aQDC == 3){
		alert('Hi, We could detect bad audio quality due to poor internet connection');	
		_aQDC=0;// Reset
              }
	   }
	 });
...
AppController.prototype.onRemoteStreamAdded_ = function(stream) {
  ...
  },7000) // start after 7 sec, this is for ICE to complete
...
PeerConnectionClient.prototype.onIceConnectionStateChanged_ = function() {
  ...
  console.log('sasi ICE state: ',this.pc_.iceConnectionState);
